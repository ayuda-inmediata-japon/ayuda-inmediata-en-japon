document.addEventListener("DOMContentLoaded", () => {
  ...
  // ======= NAVEGACIÓN ENTRE PORTADA Y BUSCADOR =======
  const inicio = document.getElementById("inicio");
  const buscador = document.getElementById("buscador");
  const botonesInicio = document.querySelectorAll('a[href="#inicio"]');
  ...
  if (arrowDown) {
    arrowDown.addEventListener("click", mostrarBuscador);
  }

  botonesInicio.forEach((boton) => {
    boton.addEventListener("click", mostrarInicio);
  });
...
<button onclick="document.getElementById('llegada-japon').scrollIntoView({behavior:'smooth'})"
style="flex:1;background:#25D366;color:white;padding:10px;border:none;border-radius:8px;">
⬆ Volver al tema
</button>
</div>
...
<p>
En Japón existen servicios especializados para cada tipo de emergencia.
Saber a quién llamar puede ayudarte a actuar rápido y correctamente.
</p>

<p><strong>☎️ Números importantes:</strong></p>
...
<p>
Si no puedes comunicarte bien en japonés, puedes usar un traductor desde tu teléfono
o mostrar esta guía a una persona cercana.
</p>
...
<p>🟢 <a href="https://chatgpt.com" target="_blank">
ChatGPT (explicar o traducir una situación)
</a></p>

<h3>📱 Apps y recursos útiles</h3>

<p>📍 <a href="https://maps.google.com" target="_blank">
Google Maps
</a></p>

<p>🇯🇵 <a href="https://www.japan.travel/es/" target="_blank">
Japan Official Travel App / Información turística
</a></p>

<p>🚨 <a href="https://www.jma.go.jp/bosai/" target="_blank">
Agencia Meteorológica de Japón / Alertas
</a></p>
...
<p><strong>Español:</strong><br>No hablo japonés.</p>
<p><strong>Romaji:</strong><br>Nihongo ga hanasemasen.</p>
<p><strong>日本語:</strong><br><span style="color:red;font-size:22px;"><strong>日本語が話せません。</strong></span></p>

<p><strong>Español:</strong><br>Por favor, llame al 119.</p>
<p><strong>Romaji:</strong><br>Hyaku jū kyū ban ni denwa shite kudasai.</p>
<p><strong>日本語:</strong><br><span style="color:red;font-size:22px;"><strong>119番に電話してください。</strong></span></p>
...
En Japón los servicios de emergencia son rápidos y confiables. No dudes en usarlos si realmente lo necesitas.
</p>
...
<p style="line-height:1.6;">
En caso de desastre, busca los puntos de evacuación señalizados:
<strong>避難場所</strong> / Hinan basho.
</p>
...
<button onclick="navigator.share({title:'Ayuda en Japón', url:window.location.href})"
style="flex:1;background:#ddd;color:#2a6edb;padding:8px 0;font-size:14px;border:none;border-radius:8px;">
↗ Compartir
</button>
...
<Img src="Img/tramites.webp"
style="width:100%;display:block;margin:0 auto;border-radius:18px;"
alt="tramites en Japon">
...
"mostrador informacion": "llegada",
  ...
  // 🔎 PALABRAS PARCIALES Y SINÓNIMOS ADICIONALES
  hote: "alojamiento",
  aloj: "alojamiento",
  hospe: "alojamiento",
  booking: "alojamiento",
  ...
  comer: "vida",
  mercado: "vida",
  "tienda 24 horas": "vida",

  tram: "tramites",
  municipalidad: "tramites",
  municipio: "tramites",
  juminhyo: "tramites",
  "my number": "tramites",
  pension: "tramites",
  licencia: "tramites",
  certificado: "tramites",
...
📞 Orientación personalizada
  ...
  function buscar() {
    ...
    const resolverClave = (consulta) => {
      const texto = normalizar(consulta);
      if (!texto) return null;

      // Coincidencia exacta primero.
      if (alias[texto]) return alias[texto];
      if (respuestas[texto]) return texto;

      const palabras = texto.split(/\s+/).filter(Boolean);

      // Coincidencia exacta por cada palabra.
      for (const palabra of palabras) {
        if (alias[palabra]) return alias[palabra];
      }

      // Coincidencia parcial desde 3 caracteres.
      // Ejemplos: hote → hotel, aloj → alojamiento, farm → farmacia.
      for (const palabra of palabras) {
        if (palabra.length < 3) continue;

        const coincidencia = Object.keys(alias).find((termino) => {
          if (termino.length < 3) return false;
          return termino.startsWith(palabra) || palabra.startsWith(termino);
        });

        if (coincidencia) return alias[coincidencia];
      }

      return null;
    };

    const clave = resolverClave(valor);
  ...
  if (searchInput) {
    searchInput.addEventListener("input", () => {
      ...
      tAuto = setTimeout(() => {
        ...
        const claveExacta = alias[valor] || (respuestas[valor] ? valor : null);

        // Solo dispara automáticamente si existe coincidencia exacta.
        // Las coincidencias parciales se ejecutan al pulsar Buscar o Enter.
        if (claveExacta && respuestas[claveExacta]) buscar();
...
/* ===== NAVEGACIÓN ENTRE BUSCADOR, APOYO Y PÁGINA FINAL ===== */
document.addEventListener("DOMContentLoaded", () => {
  const inicio = document.getElementById("inicio");
  const buscador = document.getElementById("buscador");
  const pantallaApoyo = document.getElementById("pantallaApoyo");
  const pantallaFinal = document.getElementById("pantallaFinal");
  const arrowApoyo = document.getElementById("arrowApoyo");
  const arrowFinal = document.getElementById("arrowFinal");
  const volverBuscadorApoyo = document.getElementById("volverBuscadorApoyo");
  const volverBuscadorFinal = document.getElementById("volverBuscadorFinal");

  const ocultarPantallas = () => {
    if (inicio) inicio.style.display = "none";
    if (buscador) buscador.style.display = "none";
    if (pantallaApoyo) pantallaApoyo.style.display = "none";
    if (pantallaFinal) pantallaFinal.style.display = "none";
  };
  ...
  const mostrarApoyo = () => {
    ocultarPantallas();
    if (pantallaApoyo) pantallaApoyo.style.display = "block";
    window.scrollTo({ top: 0, behavior: "auto" });
  };
  ...
  if (arrowApoyo) arrowApoyo.addEventListener("click", mostrarApoyo);
  if (arrowFinal) arrowFinal.addEventListener("click", mostrarFinal);
  if (volverBuscadorApoyo) volverBuscadorApoyo.addEventListener("click", mostrarBuscador);
  if (volverBuscadorFinal) volverBuscadorFinal.addEventListener("click", mostrarBuscador);
});
